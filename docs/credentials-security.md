...
the next 5 minutes. A successful *prompt* restamps the window, but a
silent read inside it does **not** — so the 5 minutes run from the
last time you actually touched the sensor, not from the last read.
An actively-used session re-prompts every 5 minutes, it does not stay
unprompted indefinitely.
