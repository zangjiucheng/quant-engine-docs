...
Before PER-167 these expressions bound cleanly and evaluated to NaN
...
- **No sector / beta neutralization — and the factor expression
  ...
  **The neutralization that works here is partitioning the data,
  not the score: one `research(...)` run per sector.** PER-161 is
  ...
  (PER-162; the demeaning was done offline, on an implementation
