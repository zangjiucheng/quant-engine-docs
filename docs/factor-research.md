...
It has a second, optional output: **`--export-matrix STEM`** writes the
training matrix — `STEM.csv` with the header
`bar_index,symbol,<factors in config order>,y`, and `STEM.spec.json`
with the column order, each factor's warm-up and the label horizon.

That export exists for one reason, and it is worth stating before
anyone is tempted to skip it. A model trained offline must score on
features that are **bit-identical** to the ones the engine will hand it
at inference. Recomputing `rolling_zscore` or `rsi` in pandas produces
values that differ at warm-up edges, in NaN propagation and in the
denominator's ddof — differences far too small to see and more than
enough to move a tree's split. So the engine writes the features it
actually scored on, at `%.17g` (round-trip exact for IEEE-754
binary64), and the training side reads bytes off disk rather than
recomputing anything.

Rows whose label has not happened yet are dropped rather than padded:
`y` at bar `t` is the forward return over `(t, t + horizon]`, so the
last `horizon` bars of every symbol produce no row. Carrying a zero
there would train the model on a return nobody could have earned.

The label horizon is the **first** entry in the config's `horizons`
list. Reorder the list to change it; that keeps the choice visible in
the config rather than hidden in a flag.
...
## Cross-sectional primitives inside a factor expression

`rank`, `quantile`, `is_top`, `is_bottom`, `xs_zscore`, `xs_rank_pct`
and `xs_winsorize` work in a `factor(expr = ...)`. `qe_factor` walks
the panel bar-outer / symbol-inner and runs the cross-sectional
pre-pass once per bar, exactly as the backtest and live paths do
.

Two things follow from that, and the second one saves runs:

- **A rank-preserving wrapper cannot change `xs_ic`.** `xs_ic` is a
  per-bar Spearman across symbols, so `rank(f)`, `xs_zscore(f)` and
  `xs_rank_pct(f)` all report *exactly* the same `xs_ic_mean`,
  `xs_ic_std` and `n_obs` as bare `f`, and the same `long_short`
  block too — the quantile buckets come off the same ranks. They do
  move `ts_ic`, which is a per-symbol Pearson and is not
  rank-invariant. `quantile`, `is_top`, `is_bottom` and
  `xs_winsorize` collapse values into ties and genuinely do change
  `xs_ic`.
- **This is not sector neutralization.** All seven reduce over the
  whole universe and none takes a group argument. See
  [What this layer is NOT](#what-this-layer-is-not).

These expressions used to bind cleanly and evaluate to NaN
for every symbol on every bar, which `ic_analysis`'s drop-row rule
turned into `n_obs = 0`, `xs_ic_mean = null` and a
`long_short.total_return` of exactly `0.0` at exit code 0. If you have
a `factor_report.json` on disk with that signature, it is that defect
and not a result — re-run it.
...
- `quantiles` (default `5`) — number of buckets for the long-short.
  Must satisfy `2 ≤ quantiles ≤ |symbols|`. The gate is
  `bucket_size = |symbols| / quantiles` (floor) being non-zero, so
  9 symbols in 5 buckets is fine — one name long, one short, seven
  in the middle.
...
- **No sector / beta neutralization — and the factor expression
  cannot supply it.** This page used to say "subtract a per-bar
  sector mean inside the factor expression." That was never
  possible. All seven cross-sectional primitives (`rank`,
  `quantile`, `is_top`, `is_bottom`, `xs_zscore`, `xs_rank_pct`,
  `xs_winsorize`) reduce over the **whole** universe and none takes
  a group argument; there is no `xs_mean` / `xs_sd` / `xs_median` /
  `xs_count` / `xs_sum` at all; the six bound variables are
  `close open high low volume bar_index`, so there is no sector
  label to subtract in the first place; `universe(...)` takes
  `symbols` and `data` and rejects everything else; and there is no
  cross-symbol read — `other(<symbol>, <expr>)` was deferred at
  T60.5 and never landed. Beta neutralization is out on the same
  terms: no market series is bound and there is no `rolling_beta` /
  `rolling_corr`.

  The `symbol → sector` map of EPIC-27 is not the loophole it
  sounds like. It reaches only the *reporting* side —
  `per_sector[]` in `results.json`, the HTML report's sector table,
  the dashboard's F4 breakdown — it is carried by the legacy JSON
  backtest config, not by `research(...)`, and nothing in the
  expression evaluator can read it.

  **The neutralization that works here is partitioning the data,
  not the score: one `research(...)` run per sector.** That limitation is
  the worked example — twelve single-sector cross-sections, one
  shared panel, identical settings, read as a map.

  Do not skip that and pool. Sector-demeaning the *same* 44 names
  of a balanced 4-per-sector, 11-sector pool **inverts**
  `rolling_vol`'s cross-sectional IC, `+0.01435` → `−0.00901`, and
  between-sector variance ran 28–45 % for every factor tested
  (the demeaning was done offline, on an implementation
  that reproduces this layer's own `xs_ic_mean` on the raw column).
  So a pooled multi-sector cross-section in this engine is
  measuring sector rotation unless it is demeaned, and this engine
  cannot demean it.

  The fix, when it is built, is a group argument on the
  cross-sectional primitives (`xs_zscore(expr, group = ...)`) or a
  per-symbol label vector on `universe(...)`. Neither exists today.
  Proper Barra-style neutralization is a further step again, and
