...
The dashboard reads the **flat** `strategies[]` array from
`results.json` — the loader holds it in a field called
`portfolio_strategies`, but that is not the JSON key. After portfolio flattening (nested
...
When F4 BCKT loads a `results.json` carrying a `strategies[]` array —
schema v4, or v6 as written today — the rightmost panel switches from
the monthly heatmap to `ATTRIBUTION`. It keys on the array's presence,
...
## Schema v6 result shape

`results.json` for a portfolio run is schema_version 6 (it was 4 until
EPIC-89 T89.12 added the optional `provenance` block; every v4 field is
unchanged, and 5 was already taken by walk-forward). Top-level
...
```jsonc
{
  ...
  "provenance": { ...see docs/results-json.md... }
}
```

`summary.win_rate` and `summary.avg_win_loss` are **`null`** on this
path, and print as `—` on the console. Round-trip pairing does not
decompose across N independent children, so the value was never
computed; `provenance.not_computed` names both metrics and says why.
Before T89.12 they were serialized as `0.0`, which is a value a real
