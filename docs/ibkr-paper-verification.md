...
3. Press **Cmd+Shift+X**. (The chord was `Cmd+Shift+K` until
   ...
   phase silently fail every pass criterion below.) The binding is the
   `ImGuiKey_X` handler in `apps/dashboard/gui_dashboard_app.cpp` —
   cited by symbol, because line numbers here rot.
