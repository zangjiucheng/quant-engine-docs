...
| Tab          | What's live                                                   |
| ------------ | ------------------------------------------------------------- |
| Overview     | Concentration glance, grouped by GICS sector when the market-map universe covers the names (PER-87) or `cfg.sectors` is set, per-symbol otherwise. Says how each position was priced — live quote / venue mark / AT COST — and how old the venue's marks are. |
...
the footer — PER-183 hoisted them out so the state an operator checks
...
- SAFETY footer badges:
    ...
    - **`daemon:`** — control-socket attach state. `attached`
      ...
      daemon reports on `status`: on a read-only socket (PER-65) F6
