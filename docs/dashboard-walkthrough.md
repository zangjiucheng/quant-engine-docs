...
Follow [Install & first-time setup](install.md). The first-launch wizard
creates a workspace and starter files; F3 WKSP opens that folder.
For source builds, use the build instructions in the repository README.

### Pick a broker

Start with a paper broker. Configure it in `Cmd+,`; complete the
paper verification before selecting a live account.

| Broker        | Cred source                                     | Notes |
| ------------- | ----------------------------------------------- | ----- |
| `alpaca-paper`| [OS keyring](broker-credentials.md), with environment fallback | US equities; dashboard connection. |
...
Always-on meta strip — NAV / Gross / Leverage / Long-Short / day P&L /
open P&L on the first line, then Σ Greeks (only when the book holds
options) / VaR95 / ES99 / worst stress scenario on the second — over
four sub-tabs:

There is deliberately no NET slot. The positions schema carries no cash
leg, so a "net" figure would be NAV computed twice.

| Tab          | What's live                                                   |
| ------------ | ------------------------------------------------------------- |
| Overview     | Concentration glance, grouped by GICS sector when the market-map universe covers the names or `cfg.sectors` is set, per-symbol otherwise. Says how each position was priced — live quote / venue mark / AT COST — and how old the venue's marks are. |
...
**F5 reads the attached daemon's book in preference to
`positions_json`.** `PositionsCache` has three sources — none, a
watched file, or the daemon's view of the broker — and each panel's
badge names the one it used.

The empty state distinguishes two cases that look alike and are not.
With nothing loaded it reads *"no portfolio loaded — attach a daemon,
or set `positions_json` in dashboard config (Settings (Cmd+,))"*; with
a daemon attached and a genuinely flat account it reads *"the daemon
reports a flat account — no positions to measure"*. Telling an
operator who already has a daemon to go and set `positions_json` is
advice for a problem they do not have, and it invites them to point
the screen at a stale file.

Active tab is persisted across launches.
...
With no daemon attached the SAFETY / EVAL strip is **not drawn** —
there is nothing to report and a row of em-dashes is furniture.

**Daemon attached** (normal 4-panel layout, reading from the
daemon's control socket):

```
⚠ market data is DELAYED (15 min) — rankings are computed on prices of that age
· venue marks refreshed 41s ago  ·  its clock read 16:42

SAFETY · kill: armed · reconcile: clean · session: PAPER ibkr-paper
...
The SAFETY / EVAL lines sit at the **top**, above the panels, not in
the footer — they were hoisted out so the state an operator checks
first is not the thing they have to scroll past four panels to find.
Above them is the **daemon warning banner**: one coloured line per
thing `qe_daemon status` is unhappy about — a tripped kill switch, a
market-data tier the venue granted that the daemon did not ask for,
which of the three owners paused it (watchdog, feed, or operator —
they clear differently, and naming the wrong one sends you to type a
command that does nothing), undecoded venue frames, and how old the
venue's marks are. It is empty on a healthy daemon, deliberately: a
banner that is always present is one nobody reads.
...
- SAFETY footer badges:
    ...
    - **`session:`** — dashboard's own broker session
      (legacy / pre-daemon mode).
    - **`daemon:`** — control-socket attach state. `attached`
      (green) means the socket is up and the dashboard can **read**
      state. Whether it can also **send** is a separate fact the
      daemon reports on `status`: on a read-only socket F6
      draws a READ-ONLY notice above the action row and greys out
      Stop daemon and DAEMON KILL, while the chip stays green. The
      top bar shows `RO`, plus `@host` when the daemon is on another
      machine. See the runbook's "Pointing the dashboard at it".
    - **`venue:`** — daemon's view of its broker link
      (EPIC-70). `connected` (green) /
      `PAUSED · watchdog · N missed poll(s)` (amber — soft-pause,
      reversible on reconnect; **N counts failed polls, not
      seconds**) / `TRIPPED (reason)` (red, hard trip).

      The amber chip has a second arm no earlier revision of this
      page mentioned: **`PAUSED · operator`** means someone ran
      `qe_daemon pause` and the venue link may be perfectly
      healthy — clear it with `qe_daemon resume`, not by chasing a
      disconnect. When both hold it reads
      `PAUSED · operator + watchdog`, so an operator who fixes the
      link and sees the chip stay amber has not failed to fix
      anything; their own pause is still on
      (`apps/dashboard/gui_screen_trade.cpp`, inside
      `draw_daemon_status_strip`).
...
- **DAEMON KILL row** (EPIC-88 T88.10, decision D2) — the control
  ...
  arming dies if the state changes under it. Disabled — never hidden —
  in two cases: when the control socket is down, rather than firing
  into a dead fd and reporting success; and when the attachment is
  read-only, because the daemon would refuse the verb, so the refusal
  is shown before the arming rather than after a two-click sequence. The row states, permanently and not only after
...
- **Dashboard launches with `BROKER OFFLINE` even though Gateway
  ...
  *down*. The badge flips to `PAPER` or `LIVE` when the connect
  resolves — those and `OFFLINE` are the only three values it
  takes. If it stays `OFFLINE`, check the Gateway and the log.
