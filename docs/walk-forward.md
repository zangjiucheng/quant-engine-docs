...
step_window=..., purge_bars=N?, optimize=sweep(...)?)` is the cheapest OOS
...
### When the metric cannot rank anything

Step 1 assumes `optimize.metric` produces a number for each cell.
Sometimes it produces none: `sharpe` is undefined on a
zero-variance train window, `win_rate` is undefined for a cell
that booked no round trip, and neither `win_rate` nor
`avg_win_loss` is computed at all on a multi-symbol universe
(round trips do not pair across symbols).

Two rules cover it, and neither invents a number:

- **A cell the metric measured always outranks one it did not.**
  An unmeasured cell used to win outright when it happened to be
  enumerated first, because every comparison against a NaN is
  false.
- **A window where *nothing* was rankable reports `"best_score":
  null`** and `qe_run` prints an error naming the metric and the
  window count. `best_params` still names a cell — the test slice
  has to run on something — but nothing measured it, so do not
  read it as an optimum. The OOS segment stays in the stitched
  curve either way, which is why the console says so rather than
  leaving it to whoever opens the JSON.

`optimize = sweep(..., metric = "win_rate")` on a multi-symbol
universe is refused before the run starts, since every cell of
every window would be unrankable and the whole run would produce
nothing but arbitrary parameters.
...
- **Window warmup (PER-21).** Each test slice is extended *backwards*
  by the config's own indicator warmup — the maximum `min_history`
  across every leg's entry and exit expressions — and trading is
  suppressed across that padding, so the book still starts flat and
  windows stay comparable. Before this, a window was a hard slice and
  indicators restarted from zero at its first bar: on a 252-day factor
  with a 2-year test window, 256 of 503 test bars were dead before the
  first fill. `WalkForwardRun::windows_short_of_warmup` counts windows
  the series was too short to warm; **a non-zero count means the
  estimate is still flattered** and must be read before the Sharpe is.
- **Purge is opt-in.** `purge_bars = N` drops N bars between train and
  test, belonging to neither. The default is **0**, which reproduces
  the old adjacency — so an unchanged config still has
  `test_start == train_end`, and a long-lookback factor still scores
  its first test bar from history the optimizer just trained on.
...
`best_score` is `optimize.metric` measured on the train slice for
the cell that won — the answer to "was `best_params` selected, or
just enumerated first?". It is `null` when no cell on that window
produced a finite metric (see [When the metric cannot rank
anything](#when-the-metric-cannot-rank-anything)), and always
`null` for the fixed-param runner, which selects nothing.
