...
graph TB
    ...
    subgraph "On-disk artifacts"
        ...
        JOURNAL[("events.jsonl<br/>~/Library/Application Support/<br/>qe_daemon/state/")]
...
- **Zero allocations in the hot loop.** Every `std::vector`
  ...
  upper bound. Verified by `tests/test_zero_alloc.cpp`, using the
  counting allocator in `tests/alloc_counter.{hpp,cpp}`.
...
over a control socket — `~/Library/Application Support/qe_daemon/daemon.sock`
by default, or whatever `daemon_socket_path` names — polls
daemon-side state every 3 s, and renders it through the F6 panels
(Working Orders / Executions / Account / Order Log).

The daemon serves **two** endpoints. The full socket accepts every
verb; a second, read-only one serves observation only and
refuses anything that would change state. An SSH-forwarded read-only
socket is how the dashboard watches a daemon on another machine
without being able to touch it, and the daemon reports which kind the
client reached so the UI can grey out controls rather than offer ones
...
disconnects the broker and stops the strategy mid-session. Note
what it does *not* do: **the orders you already have working at
IBKR stay working.** Nothing in this codebase cancels them, and
the venue does not cancel them for you when a client goes away —
so closing the dashboard would leave live orders running with
nothing watching them. Putting the session in a long-running
daemon is the only safe design for headless / overnight
strategies.
...
**`is_connected` means BOTH venue links**. The daemon holds
two independent TCP connections to TWS on two client ids — the data
link on `live.ibkr.client_id` and the order link the broker adapter
owns on `client_id + 1` — and the predicate is their AND. It was the
data link alone, so an order socket that died while quotes kept
arriving left the watchdog Healthy and the daemon went on submitting
into a socket that was gone. `status` reports
`data_link_connected` / `order_link_connected` separately, because a
dead data link and a dead order link need different remedies.
...
| `Paused → Tripped` | `KillSwitch::trip("ibkr_disconnect")` + a journaled count of what is still working at the venue — see below. Local pending slices are **preserved**, not cancelled |

- **Healthy** → normal operation.
- **Paused** → new orders reject (`PreTradeRisk` gate). Open
  orders untouched. Auto-resumes on reconnect within the trip
  window. Reversible.
- **Tripped** → KillSwitch tripped, pending (never-submitted)
  staged slices **preserved** (they cannot fire while killed; a
  restart re-evaluates them), daemon needs restart. Sticky. The
  daemon does **not** exit; it keeps consuming data and booking fills.
- **Orders already working at the venue survive a trip.** A trip
  blocks *new* submissions; it does not reach the venue. This is a
  deliberate design decision (EPIC-88 D1), not a gap: cancelling is
  an explicit operator act, never a side effect of a trip.

    Until EPIC-88 the callback did try `list_open_orders()` →
    `cancel_order()` here. That loop was **deleted rather than
    repaired** (T88.2), because the only trip reason that reaches
    this callback is `ibkr_disconnect` — at which point the
    connection is down, the list call returns "not connected", and
    it returned early. It never cancelled an order and could not.
    What replaced it is an honest report: the callback asks the
    broker how many orders are working, logs the answer, and
    journals a `venue_orders_left_working` row carrying either a
    count or the literal `unknown (broker link down)`. A
    post-mortem has to be able to tell "there were none" from "we
    could not find out", and the old code made those identical.

    To actually cancel, use `qe_daemon cancel-all` or the F6
    **CANCEL ALL** control (EPIC-88 T88.1) while the link is up, or
    cancel by hand in TWS / IB Gateway / Client Portal. Note that
    after an `ibkr_disconnect` trip the link is by definition down,
    so the manual route is the only one available until it
    recovers. See
    [Live-trading safety, Layer 3](live-trading-safety.md#layer-3-kill-switch).

The dashboard's F6 `venue:` badge reflects the state directly:
`connected` (green) / `PAUSED · watchdog · N missed poll(s)` (amber; N counts failed polls, not seconds) /
`TRIPPED (reason)` (red).
...
graph TB
    ...
    subgraph "~/Library/Application Support/qe_daemon/"
        ...
        DAEMON_STATE["state/<br/>events.jsonl + events-UTC.jsonl<br/>(flat; override per deploy<br/>with live(journal_dir = ...))"]
...
- **Author a strategy** — [`first-factor-tutorial.md`](first-factor-tutorial.md):
  10-minute walkthrough, entirely in the dashboard — no clone, no
  terminal, starting from F3 WKSP.
...
- **Forecasting layer** — [`forecasting.md`](forecasting.md): the
  `predict_return` API. Ridge and lasso fit online, every bar; `gbdt`
  loads a model trained offline in Python and only does the forward
  pass, which is why it takes `model` and refuses `lookback`.
