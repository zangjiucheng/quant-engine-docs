...
| `Paused → Tripped` | `KillSwitch::trip("ibkr_disconnect")` + `OrderScheduler::cancel_all` (local pending slices) + a journaled count of what is still working at the venue — see below |
...
- **Tripped** → KillSwitch tripped, pending (never-submitted)
  staged slices dropped, daemon needs restart. Sticky. The daemon
  does **not** exit; it keeps consuming data and booking fills.
