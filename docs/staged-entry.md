...
- `slice_cancelled` — kill-switch trip, operator cancel, or
  window-miss skip. **A graceful `qe_daemon stop` is not on that
  list, and that is deliberate** (2026-07-31, reversing T83.8): the
  ...
  slice: a kill-switch trip drops the local queue, it does not
  cancel anything at the venue, so a slice that already reached
  `slice_fired` stays working at the broker. To pull those, use
