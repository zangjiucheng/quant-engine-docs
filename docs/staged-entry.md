...
### `staged_entry(slices, on_partial_fill?, on_window_miss?, open_offset_s?, close_offset_s?, window_tolerance_s?)`
...
- **`on_window_miss`** (`"skip"` | `"reschedule_next"`, default
  `"skip"`) — what to do when a slice is late by more than
  `window_tolerance_s`. See below; **enforced since EPIC-83**, and
  `reschedule_next` is still parsed-but-unimplemented.
...
- **`window_tolerance_s`** (non-negative integer, default `300`) —
  how late a slice may fire and still count as inside its window.
  `0` means "use the 300 s default", not "no tolerance". A negative
  value is a load-time error.

#### Window-miss enforcement (EPIC-83 T83.8)

`on_window_miss` was parsed and promised by three documents but
**read by no code** until EPIC-83. A slice whose window had passed
fired anyway — including one hydrated from a journal days later,
which meant a market order at an arbitrary hour, possibly outside
regular trading.

`OrderScheduler::tick()` now checks lateness on every Pending slice
that is due:

- Inside `window_tolerance_s` → submit normally.
- Past it → the slice is **Cancelled**, never submitted, and a
  `slice_cancelled` row lands with `reason = "window_miss"`. The
  engine's leg is re-armed so the next evaluation round can decide
  afresh.
- `on_window_miss = "reschedule_next"` degrades to the same
  cancellation but with the distinct reason
  `window_miss_reschedule_unimplemented`, so the journal never
  claims a reschedule that did not happen.

The lateness check runs **before** the pause check, deliberately.
A slice past its window is dead regardless of *why* it is late; if
pause won the ordering, a long operator pause would preserve a
stale slice and fire it hours outside its window the moment trading
resumed. A pause that ends within the tolerance still fires the
slice normally.

Five minutes is wide enough to absorb scheduler jitter, a short
pause and a daemon restart, and far tighter than the gap between two
sessions — so a stale hydrated slice is always retired rather than
hitting the venue. Widen it only if you have a specific reason;
`window_tolerance_s` is the difference between "we were briefly
slow" and "this order no longer reflects a decision anyone made".
...
- `slice_cancelled` — operator cancel, or window-miss skip.
  **Neither a graceful `qe_daemon stop` nor a kill-switch trip is on
  that list, and that is deliberate** (2026-07-31, reversing T83.8): the
  queue survives a restart and rehydrates, because throwing away a
  session's orders every time the daemon is restarted in the evening
  cost more than it ever prevented. A slice that comes back after its
  window is retired by the window-miss rule above, so a stale one
  still cannot be submitted late.
  Only ever a **Pending** (never-submitted)
  slice: a kill-switch trip preserves the local queue and does not
  cancel anything at the venue, so a slice that already reached
  `slice_fired` stays working at the broker while a still-Pending one
  waits for the next start. To pull those, use
  `qe_daemon cancel-all` or the F6 **CANCEL ALL** control
  (EPIC-88 T88.1), or cancel by hand in TWS. Note the asymmetry:
  cancel-all reaches **venue orders only** — staged slices still
  Pending are not venue orders and it does not touch them, which
  is why its reply reports `staged_pending` separately. See
  [Live-trading safety, Layer 3](live-trading-safety.md#layer-3-kill-switch).

Since EPIC-83 each of the three rows also carries a `strategy_id`
tag, and the journal is one flat file shared by every config that
has ever run against the same `journal_dir`.
...
`slice_scheduled` rows tagged with a *different* `strategy_id` are
skipped, so two configs sharing a journal no longer adopt each
other's queued orders. Rows with **no** tag are pre-EPIC-83 and are
adopted (there is no other way to recover them) and counted — the
daemon logs the count once at startup. A Pending row hydrated from
a long-dead run is then retired by the window-miss check on the
first tick rather than being submitted at market.
