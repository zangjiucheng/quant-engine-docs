...
|---|---|---|
| **config** | Top-level + every arg position except the deferred ones — `signal(entry=…/exit=…/size=…)` and `factor(expr=…)` | Let bindings, kwarg calls, ranges, arrays, strings, arithmetic, the config builtins below |
| **signal** | The deferred args: `signal(entry=…/exit=…/size=…)` and `factor(expr=…)` | Per-bar variables (`close`, `open`, `high`, `low`, `volume`, `bar_index`); signal-layer functions; arithmetic / comparison / logical ops; positional args only — no let, no arrays. Kwargs and string
  literals exist in exactly one place: `predict_return(...)`, whose
  kwarg form takes `method` and `model` as strings, read once at bind
  time rather than per bar |
...
since 0.4.0 the lint is a **hard refusal at the live boundary only**:
...
`yahoo(...)` fetch of the same symbol therefore agree.

**There is no other format.** This paragraph used to offer
`format = "generic"` "for a file that is already on whatever basis you
want". `CsvFormat::Generic` is declared in `io/csv_loader.hpp` and
implemented nowhere, and the binder does not accept the string — so that
sentence documented an option that has never existed. Corrected
2026-09-16 while building the IBKR cache on top of this
contract.

For data that is **already** on the basis you want — IBKR's historical
bars, for instance, which the venue adjusts itself — write `Adj Close`
equal to `Close`. The adjustment step then applies a ratio of exactly 1
and the proven path carries the file unchanged, which is a smaller thing
to trust than a second format.
...
> **The default `compound = false` has a cost, and the run now names
> it.** Entries stay keyed to the initial capital, so a book
> that doubles its equity is risking half as much per dollar by the
> end. When the range of `equity / initial_capital` across a run's
> entries reaches 1.25×, `qe_run` attaches a `sizing_basis_decay`
> warning to the summary block and to `provenance.warnings` in
> `results.json`. It is a warning and not a refusal: the default is
> correct — the daemon will not deploy `compound = true` at all — but
> a drawdown measured on a de-risking book is not comparable to a
> constant-risk benchmark, and that mistake has been published once.
> See [Backtest/live parity](safety-and-limitations.md#compound-true-has-no-live-equivalent).
...
- `condition` — any expression that evaluates to a number or bool
  ...
  arithmetic and let-bound scalars work. **String equality does
  not**: the comparison arm coerces both operands through
  `as_number`, which accepts only numbers and bools, so
  `assert(sym == "SPY", ...)` aborts the load before the assert
  handler ever runs.
...
|---|---|---|
| `rank(<expr>)` | integer 0..M-1 | Ascending rank of `<expr>` for the current symbol among the **M scored** symbols. Bucket 0 = smallest. |
| `quantile(<expr>, q)` | integer 0..q-1 | Which quantile bucket of the **scored** symbols the current one falls into. Bucket 0 = smallest. |
...
maintain per-symbol state independently. As of T89.19 nested
cross-sectional (e.g. `rank(rank(close))`) **binds and runs** too:
sites are numbered in post-order, so an inner site always has a
lower slot than the site reading it and is computed first within the
same pre-pass.

All seven primitives agree on who is a **scored** member: a symbol
whose inner expression is finite this bar. A symbol still warming up
(NaN) is not scored, and neither is one whose value came out infinite
— `lag(close, 21) / lag(close, 252) - 1.0` divides by a price the CSV
loader reads as `0.0` from an empty field, so this is not a
hypothetical. An unscored symbol gets the per-kind "no signal"
sentinel: `rank` / `quantile` / the three normalizers return NaN;
`is_top` / `is_bottom` return 0 so the `entry_val != 0.0` gate in the
strategy correctly says "no entry".

The unscored are then **removed from the question**, not just from the
answer: with M of N symbols scored, `rank` runs 0..M-1, `quantile`
splits those M into `q` buckets, and `is_top(x, k)` names k of the M
(or nobody, if `k > M`). An unusable symbol therefore costs its own
signal and nothing else — it does not push a survivor up a rank, and
it does not reserve a top bucket that no scored name can reach.
...
symbol per timestamp. **Nesting is allowed** as of T89.19,
normalizers included — `xs_zscore(rank(x))` and
`is_top(xs_zscore(x), 10)` both bind and run. Sites are numbered in
post-order, so the inner site always holds a lower slot than the one
reading it and is resolved first; the pre-pass copies the
already-computed lower-slot values into the inner evaluators rather
than allocating a second pass. The refusal that stood here until
T89.19 cost nothing it claimed to save, and was not enforced anyway:
`signalize_universe` hand-appends its `is_top` site after binding, so
composite scores were already being assembled in exactly this shape,
never came through the binder, and silently scored NaN for every
symbol. The binder refused what an author typed and permitted what the
config layer built.
...
### `walk_forward(base, train_window, test_window, step_window, purge_bars?, optimize?)`
...
- `purge_bars` — optional bar count, default `0`. Bars dropped
  between the end of train and the start of test, belonging to
  neither slice. Without one, `train_end == test_start`: a label
  computed near the end of the train window looks *forward*, so a
  feature on the last train bar and a return realised in the first
  test bars are the same event, and the model is scored partly on
  what it was fitted on. Set it to at least the longest forward
  horizon your labels use. In **bars**, not a duration — the thing
  being protected against is counted in observations, and a
  duration would have to be re-derived against each symbol's
  calendar to mean anything. The pinned dataset in
  `data/ibkr/2026-09-16/splits.json` uses `purge_bars = 126`, its
  longest feature lookback.

  Each test window is also **warmed** before it is scored: the
  slice is extended backwards by the config's own indicator
  warmup, with trading suppressed across the padding, so the book
  still starts flat. The results file records `purge_bars`,
  `warmup_bars` and `windows_short_of_warmup` under
  `strategy.params` — all three even when zero, so a run that
  could not warm a window says so instead of looking warm.
- `optimize` — optional `sweep(...)`. When set, each train slice
  re-runs the sweep, picks the best params by `optimize.metric`,
  and applies them to the test slice. A cell whose metric is not
  finite is not ranked at all — it can never beat one that was
  measured — and a window where no cell was rankable reports
  `"best_score": null` plus a console error rather than passing
  off the first enumerated cell as the winner. `metric =
  "win_rate"` is rejected outright on a multi-symbol universe,
  where it is not computed. See [When the metric cannot rank
  anything](walk-forward.md#when-the-metric-cannot-rank-anything).
...
downstream consumers detect fixed vs. optimized. `best_score` is
`null` there too, and on any optimized window where the metric
ranked nothing.
...
- `maintenance_window_utc_minute` (`0`–`1439`) and
  `maintenance_window_minutes` (`1`–`1439`) — **both or neither**,
  off by default. Declares a daily window, in minutes past UTC
  midnight, during which a broker disconnect is *expected*.

  IB Gateway exits once a day for its own session maintenance. Without
  this, the disconnect watchdog escalates that to a one-way kill in 27
  seconds, and clearing a kill needs a daemon restart — so a scheduled
  vendor event costs a restart and a reconcile ack every night.

  Inside the window a disconnect still soft-pauses, still probes,
  still journals and still counts misses. What it does not do is
  **trip**. A link still down when the window closes trips on the very
  next poll, so the kill is deferred rather than cancelled
  (`src/live/disconnect_watchdog.cpp:169-189`).

  Set it deliberately: it weakens the control that catches a real
  venue outage, for a fixed period, every day. The deployed config
  uses `1425` (23:45 UTC) for `45` minutes, which covers IBKR's
  nightly exit with margin.
...
- `universe` — one `universe(...)` value. **Kwarg only**:
  `research(...)` rejects positional arguments outright, so
  `research(universe(...), factors = [...])` is an error, not a
  shorthand.
...
DSL functions, positional args only — except `predict_return(...)`,
which also accepts a kwarg form, and requires it for
`method = "gbdt"`:
...
1. **Bootstrap a workspace**: launch `qe_dashboard`. On a fresh
   install it drops nine `example_*.qe` starters into
   `<workspace>/backtests/`, two into `<workspace>/sweeps/`
   (`example_sweep_spy.qe` and `example_sweep_paired_dates.qe`), and
   a `positions/example_positions.json` — twelve files in all.
