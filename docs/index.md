...
- **Want the app?** [Download QE Dashboard for macOS](download.md).
  Apple Silicon only, and macOS blocks the first launch — the page
  covers both, up front.
...
- **About to trust a backtest number?**
  [Safety & known limitations](safety-and-limitations.md) lists what
  this engine gets wrong, does not model, or models in a way that will
  surprise you — survivorship bias, backtest/live divergence, and the
  incidents that put each item on the list.
...
| **Forecast** returns — rolling ridge / lasso, or a pre-trained GBDT | [Forecasting](forecasting.md) |
...
| Know what the engine **gets wrong**         | [Safety & known limitations](safety-and-limitations.md) |
