...
| ← server  | 7   | `PORTFOLIO_VALUE`     | Per-symbol marks / open P&L |
| ← server  | 8   | `ACCT_UPDATE_TIME`    | The venue's stamp on the account snapshot |
| ← server  | 58  | `MARKET_DATA_TYPE`    | Tier the venue granted |

**`ACCT_UPDATE_TIME` is the one to read carefully.** It carries an
`HH:MM` with **no date and no timezone**, so it cannot be turned into
an instant without guessing both. It is kept as the string the venue
sent, paired with the LOCAL time the frame arrived, and the age an
operator reads is measured from the receive time — see
`IbkrConnection::MarksStamp`. A mark's age is the only thing that
distinguishes "the position is worth \$983.85" from "it was worth
\$983.85 at 05:09".
...
3. **Re-dial, up to a point**. An ordinary `disconnect()`
   stays down, but an unexpected death with a socket factory
   installed enters `reconnect_loop()` — up to `kMaxAttempts = 30`
   with a backoff from `kBaseDelayMs = 1000`
   (`src/net/ibkr_connection.cpp:850-868`). The state goes
   Disconnected *first* and stays there for the whole attempt, so
   `is_connected()` is false throughout: the watchdog soft-pauses
   and `submit_order` refuses at its guard. This was added because
   IB Gateway's nightly scheduled exit was tripping the one-way
   kill switch every night.

   We still never auto-restart *TWS itself* — that is the claim
   the safety doc makes, and it is unchanged. If the loop is
   exhausted the user re-launches Gateway and the dashboard's
   `Reconnect IBKR` action triggers a new `IbkrBroker::connect()`.
