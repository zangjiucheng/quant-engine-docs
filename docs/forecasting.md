...
> and the gradient-boosted `gbdt` method.
>
> The three are not the same shape. Ridge and lasso **learn online**:
> every bar pushes a training pair and refits, so the score at bar `t`
> reflects data through `t-1`. `gbdt` is **trained offline in Python and
> fixed at load** — it never learns during a run. That difference is why
> `lookback` is **rejected** for `gbdt` rather than merely unused: it is
> the rolling fit window, a boosted model has no fit, and accepting a
> number the run discards is what makes a config look tuned when it is
> not.
...
- **v2 kwarg** (EPIC-50a + EPIC-50b) — `predict_return(lookback =
  ...
  target = E, method = "ridge" | "lasso" | "gbdt", model = "<path>")`.
  Unlocks these extensions:
...
| `f1..fN`     | signal expr   | at least one; at most 64             |
...
|------------|------------------|--------------|-------------------------------------|
| `lookback` | int literal      | required for ridge / lasso; **rejected** for gbdt | `>= n_features + 1` (linear solvers only — gbdt has no fit window, so the under-determination guard is skipped) |
| `features` | array literal    | — required   | `[expr, expr, ...]`, 1 to 64 entries |
| `method`   | string literal   | `"ridge"`    | `"ridge"`, `"lasso"` or `"gbdt"`    |
| `model`    | string literal   | —            | required iff `method = "gbdt"`, rejected otherwise; path to the offline-trained ensemble, loaded and validated at **bind** time, and its `n_features` must equal `features.size()` |
| `alpha`    | double literal   | required if `method = "ridge"`; rejected for lasso and gbdt | `>= 0` |
| `lambda`   | double literal   | required if `method = "lasso"`; rejected for ridge and gbdt | `> 0` |
| `horizon`  | int literal      | `1`          | `>= 1`                              |
| `target`   | signal-context expression | next-bar close-to-close return | causal (backward-looking) |

`method` and `model` are the only two signal-context kwargs that accept
a string literal — strings remain rejected everywhere else in the signal
layer. Both are read **once at bind time** rather than evaluated per
bar, which is what makes them safe exceptions rather than a widening of
the rule. The binder gives a targeted error if a string sneaks into any
other position.
...
prediction with `target = realized_vol(close, 20)`, or longer-horizon
return prediction with `target = lag_return(close, 5)` paired with
`horizon = 5`.

> This example named `rolling_vol(close, 20)` until EPIC-86. That is
> the std-dev of the **price level in dollars**, not of returns — a
> model trained on it predicts price level, not volatility. Use
> `realized_vol`. See
> [Feature primitives](#feature-primitives) below.
...
**Method (`method = "ridge" | "lasso" | "gbdt"`)** — pick the solver. Ridge
...
**`gbdt`** is the odd one out and is documented as such. It does not
fit anything at run time: the ensemble is trained offline in Python,
serialised to JSON, and loaded once at bind time. `push()` and `fit()`
on it are deliberate no-ops, so unlike ridge and lasso its score at bar
`t` depends only on that bar's features and not on any history.

```qe
# No lookback, no alpha, no lambda — all three are refused here,
# because there is no fit window and nothing to regularise.
predict_return(
  method   = "gbdt",
  model    = "models/it30_h20.json",
  horizon  = 20,
  features = [
    lag_return(close, 1),
    lag_return(close, 20),
...
The model file is **untrusted input on the path that decides trades**,
so it is validated rather than trusted: tree and node counts, depth,
feature indices, non-finite thresholds and leaf values, and — the
invariant that matters most — every child index must exceed its parent,
because a backward edge is a cycle and a cycle silently drops a whole
tree out of the ensemble. A malformed file is refused with the reason
and the offending index named, never repaired.

The features it scores must be **bit-identical** to the ones it was
trained on. That is what `qe_factor --export-matrix` exists for: the
engine writes the training matrix at `%.17g`, and Python reads bytes off
disk rather than recomputing a single feature in pandas. See the
...
| `rolling_vol(x, n)`        | n-window sample std **of `x` itself**; NaN n-1 bars |
| `realized_vol(x, n)`       | n-window sample std of `x`'s 1-bar returns; NaN n bars |
| `rolling_zscore(x, n)`     | `(x_t - mean_n) / std_n`; 0 on constants   |
| `price_level(x)`           | identity; marks a deliberate price-level argument |
...
reasonable window. `price_level` is pure and stateless.

**Watch the units.** `rolling_vol` returns the std-dev of whatever it
is fed, so `rolling_vol(close, 20)` is in **dollars** and tracks price
level (rank correlation +0.82 with `close`, +0.12 with true return
volatility). `realized_vol(close, 20)` is the dimensionless per-bar
return volatility, and is bit-identical to
`rolling_vol(lag_return(close, 1), 20)`. Neither is annualized.
Feeding a dollar-valued feature into a ridge model alongside
dimensionless ones also wrecks the regularization balance, so this is
not only a semantics question. Passing a bare price variable to
`rolling_vol` emits a config lint at load — see
[`qe-language.md`](qe-language.md#volatility-rolling_vol-vs-realized_vol).
...
Read the resulting `results.json`. The comparison is **per window**,
not aggregated — `walk_forward` carries exactly `n_windows` and
`windows[]`, and each entry has both halves:

```
walk_forward.windows[i].in_sample_metrics.sharpe
walk_forward.windows[i].out_of_sample_metrics.sharpe
```

If in-sample sharpe is high across windows but the out-of-sample
sharpe sits near zero, the predictor is fitting noise. This is the
canonical overfit smell-test the forecasting layer is designed to
support.
...
- **Targets**: `target = ...` (EPIC-50a) lets you predict
  ...
  head) is not supported by any method here — `gbdt` scores a
  regression leaf sum, not a class probability.
- **Methods**: ridge (default), lasso (EPIC-50b) and `gbdt`
 ship today. Neural-net inference via ONNX was planned
  and **deliberately dropped** — see `EPICS/done/EPIC-51-tree-and-nn-predictors.md`.
  Bayesian regression is not planned.
- **`gbdt` does not train.** The engine links no training library and
  never will: `xgboost` is deliberately absent from `vcpkg.json`, and
  only the traversal ships. Training happens offline, and the features
  it trains on must come from `qe_factor --export-matrix` rather than
  being recomputed in pandas — a feature that differs by 1e-9 between
  training and inference is a model scored on a distribution it never
  saw, and nothing in the backtest would say so.
...
| Boosted-tree loader + forward pass | `include/qe/forecast/gbdt.hpp`<br>`src/forecast/gbdt.cpp` |
| Training-matrix export     | `include/qe/io/feature_matrix.hpp`<br>`src/io/feature_matrix.cpp` |
| Feature indicators         | `include/qe/indicators/lag_return.hpp`<br>`include/qe/indicators/rolling_stats.hpp`<br>`include/qe/indicators/realized_vol.hpp` |
...
- **64 features is a hard ceiling, not a guideline**: the per-bar
  feature vector is built in a fixed-width stack array so the hot loop
  allocates nothing. The binder rejects a 65th feature outright
  (`BoundAst::kMaxPredictFeatures`). The ceiling used to be
  documented but unenforced, and a 65-feature call site bound cleanly
  and then wrote past the end of that array on the first bar.
