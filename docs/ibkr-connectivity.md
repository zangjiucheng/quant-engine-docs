...
- Drop the socket on session timeout if no traffic for >30s.
  We send no application-level keepalive — there is no
  `reqCurrentTime` in the outbound message set. The socket relies on
  `SO_KEEPALIVE` and on the steady inbound tick traffic of an active
  subscription.
...
`client_id = 17` by default (`DashboardConfig::ibkr_client_id`)
— deliberately not 1, which is the id most sample code and most
other tools reach for first. If you also use TWS API from a
notebook or another tool at the same time, set that other tool
to a different id to avoid stepping on each other's
order acks.
...
| Messages per second | ~50 / client id | We do not rate-limit by default: `SafeBrokerLimits::max_orders_per_minute` is `0` (off). The daemon has a separate throttle you opt into with `live(risk_max_orders_per_minute = N)`, which gates `PreTradeRisk` rather than the broker wrapper. |
...
| `Already an API session with this client ID` | Another tool (or a previous dashboard run that didn't disconnect cleanly) is using the same id — 17 unless you changed it | Settings (Cmd+,) → change client_id, or kill the other process |
