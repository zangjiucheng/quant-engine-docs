...
#### The decay is now measured, and the default still does not flip (PER-22)

PER-22 asked whether `compound` should default to **true** instead, on
...
### A third runtime: walk-forward windows — warmed since PER-21, purge is opt-in
...
- ~~**No warmup padding.** A window is a hard slice; indicators restart
  from zero state at its first bar.~~ **Fixed (PER-21, 2026-09-17).**
...
Multi-symbol **walk-forward** was the one path left reporting `0.0`,
because that value fed the parameter-selection argmax; PER-11 changed
the argmax and the value together, so it reports `null` there too. See
...
shorter book (`src/net/ibkr_connection.cpp:423-447`, `:1577-1595`), so
...
rather than claiming a null.~~ **Superseded (PER-11).** That path was
...
~~Documenting it was the whole remedy.~~ **Superseded by PER-22.** The
...
the engine would have volunteered it. As of PER-22 the run measures its
...
open, and the default is still `false`; see [the PER-22
section](#the-decay-is-now-measured-and-the-default-still-does-not-flip-per-22)
