...
| Term-structure of risk-free rate | One scalar `r`. Default `0.045` (set in `DashboardConfig::risk_free_rate`). |
...
position file with a past expiry. The **math layer** returns intrinsic
value and Δ at {−1, 0, +1}, as the `T <= 0` row above says.

The **dashboard** does not use it: `risk_snapshot` checks `t <= 0`
first, marks the leg `"expired"` and returns before pricing, so the
leg stays unpriced and is **excluded from `option_nav`** — only
`priced` legs are summed. An expired leg therefore shows as expired
and contributes nothing, rather than quietly carrying a stale
intrinsic into the NAV. Edit the position file to make it count
again.
...
1. **Within `quote_mark`**, before the solver runs at all: mid when
   `bid > 0 && ask > 0 && ask < 2 * bid`, else `lastPrice` when
   `> 0`, else NaN. There is no "retry" — the solver is invoked at
   most once, on whichever mark survives this.
2. If that yields no usable sigma, fall back to a **leg-supplied IV
   from the positions JSON** (`leg.iv`), noted as `leg IV`. This
   keeps Greeks live during the first-fetch gap. It is *not* Yahoo's
   `impliedVolatility` — the chain's own IV is never consulted.
3. Otherwise the leg is left unpriced, note `no quote` or
   `loading…`, and returns `NaN` IV → Greeks render as "—"
