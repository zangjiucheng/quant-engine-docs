...
[warn]  watchdog: cancelled N pending staged slice(s)          # only if N > 0
...
6. **Kill-switch cancels Pending — and only Pending.** Trip the
   kill-switch with at least one slice still Pending. The watchdog
   log emits `watchdog: cancelled N pending staged slice(s)` and
   the panel's pending rows flip to `cancel` (red). No broker
   submissions occur for cancelled slices.
