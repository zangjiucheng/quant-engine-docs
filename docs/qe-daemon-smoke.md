...
```bash
./build/release/bin/qe_daemon status --json
```
...
```
SAFETY     ⚠ PAUSED
  pause      PAUSED by watchdog · ibkr_disconnect_soft
```
...
[warn]  watchdog: list_open_orders failed during disconnect cleanup: ibkr_connection: not connected
```

> **That last line is the expected outcome, not a smoke failure.**
> The on-trip callback tries to cancel working orders at the venue,
> but the reason it fired is that the venue is unreachable — so
> `list_open_orders` returns "not connected" and the callback
> returns early (`apps/daemon/main.cpp`, the `if (!open)` guard in
> `cancel_open_on_trip`). **No order is cancelled at IBKR by this
> path, ever.** The staged-slice line above it is real work: those
> are locally queued slices that were never submitted.
>
> If you had a working order at IBKR when the link died, it is
> still working. Cancel it in TWS / IB Gateway / Client Portal.
> See [Live-trading safety, Layer 3](live-trading-safety.md#layer-3-kill-switch).

`qe_daemon status` now reports

```
SAFETY     ⛔ KILL TRIPPED — no new orders will be submitted
  kill       TRIPPED (one-way) · ibkr_disconnect
```
...
6. **Kill-switch stops Pending from firing — and preserves it.**
   Trip the kill-switch with at least one slice still Pending. The
   watchdog log emits `watchdog: N staged slice(s) PRESERVED across
   the kill` and the pending rows **stay Pending** in the panel. No
   broker submissions occur while the gate is closed.
...
- ~~**`qe_daemon pause` / `qe_daemon kill` CLI subcommands**~~ —
  **fixed in EPIC-88.** `kill` / `pause` / `resume` (T88.11) and
  `cancel-all` (T88.1) are real subcommands now, and F6 sends
  `kill` (T88.10) and `cancel_all` (T88.1). Until then the verbs
  existed on the socket and nothing that shipped could send them:
  an unknown first argument fell through to `start`, so
  `qe_daemon kill` tried to *launch a second daemon* using the
  string `"kill"` as a config path and failed seconds later on the
  single-instance lock. The fall-through is now narrowed — a first
  token with no path separator and no `.qe` suffix is an error that
  lists the verbs.
- ~~**Multi-symbol live** — `LiveEngine` is single-strategy /
  single-trade-symbol.~~ **No longer true, and it contradicted this
  same file.** `LiveEngine` holds a `std::vector<Leg>`, each with its
  own `trade_symbol`, and dispatches bars by matching
  `bar.symbol == leg.trade_symbol`. Multi-leg and `signalize_universe`
  configs run live — see the cross-sectional barrier section above and
  the runbook's "Multi-leg + cross-sectional".
