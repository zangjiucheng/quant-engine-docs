...
`setup_completed == false`) opens a wizard of seven screens — a
Welcome and a Done bookending five numbered steps:
...
5. Command line — putting `qe_run` / `qe_daemon` on your `PATH`.
6. Starter file — which example `.qe` to open first.
7. Done

Back/Next walk the screens one at a time with no conditional skip, so
you always see all seven.
...
```bash
BROKER="/Applications/QE Dashboard.app/Contents/Resources/setup-broker.sh"

"$BROKER" --list                # shows configured creds
"$BROKER" --check ibkr-paper    # tcp probe + creds check
qe_daemon --version             # if you opted into CLI PATH
```

The script ships inside the bundle, next to the CLI tools above. (In a
source checkout the identical copy is `scripts/setup-broker.sh` — this
page used to give only that path, which does not exist for anyone who
...
- [`.qe` language](qe-language.md) — configuration and builtin reference.
- [Daemon smoke checklist](qe-daemon-smoke.md) — deployment verification.
- [Forecasting](forecasting.md) — ridge, lasso and GBDT signals.
- [Broker credentials](broker-credentials.md) — `qe_creds` usage.
