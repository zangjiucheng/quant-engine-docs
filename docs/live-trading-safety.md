...
>
> **The same flag arms a second refusal, on the other book** (PER-144).
...
- The watchdog trips the kill-switch, so no new order goes out.
  ...
  them anyway. Unfired staged slices are **preserved**, not
  dropped: they cannot fire while the gate is closed, and a
  restart re-evaluates them, firing those still inside their
  window and retiring the rest as `window_miss`. There used
