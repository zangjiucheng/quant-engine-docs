...
EPIC-75 hardens that path with five composable capabilities driven by
a single new canonical record — the `EvalRound`. **Four are live and
enabled by default; pre-trade replay is not reachable — see below.**
...
| **Pre-trade replay**   | ⚠️ **Library only — no trigger exists.** See below     |
...
!!! warning "Not reachable today"
    The replay *pass* — `run_replay_pass`, the only thing that emits
    an `eval_replay_mismatch` row — has **no production caller**. Its
    only callers are in `tests/test_eval_replayer.cpp`. EPIC-75
    shipped T75.8 (the function) but never T75.9 (the pre-open
    worker) or T75.10 (its config knobs), and the planned
    `qe_daemon eval --replay <close_ts>` subcommand does not exist
    either. There is no verb, CLI, worker or button by which an
    operator can produce a `Verified` or `Mismatch` outcome.

    What IS live is `replay_eval_for`, which `EvalBackfiller` calls
    on the backfill path (see [Backfill](#backfill-same-evening-recovery-p4b)). The mechanism,
    the read-only `CapturingNullRouter` guarantee and the journal
    event kind are all real and unit-tested — only the trigger is
    missing.

The design, for when a trigger is added:
...
| ~~Pre-open replay lead~~ | — | **Not implemented.** No such knob exists in the tree; the pre-open replay worker was never built. |
...
The right starting point for any of these is the journal. There is no
automatic per-config subdirectory — the path below is the default. A
deployment that sets `live(journal_dir = "name")` gets
`.../state/name/events.jsonl` instead, and an absolute `journal_dir`
is used as given:

```bash
tail -F ~/Library/Application\ Support/qe_daemon/state/events.jsonl |
