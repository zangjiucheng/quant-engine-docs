...
arg_list    = arg , { "," , arg } ;
arg         = expression
            | ident , "=" , expression      (* kwarg *)
            ;
array       = "[" , [ expression , { "," , expression } ] , "]" ;
string      = '"' , { any_char_except_quote } , '"' ;   (* no escapes *)
...
| `price_level(x)` | `x` (identity)         |

`price_level` is an exact identity with no state slot and
`min_history == 1`. It exists only to mark a price-level argument as
deliberate and silence the `rolling_vol` lint below.
...
`rolling_vol`, `realized_vol`, `rolling_zscore`) and feature
primitives live in [`forecasting.md`](forecasting.md) — same per-bar
evaluator, but documented alongside the rolling-ridge math they exist
for.

## Bind-time lints

`qe::dsl::bind()` returns non-fatal findings in `BoundAst::lints`
alongside the bound AST. Binding still **succeeds**; the caller
decides whether to surface them. `qe::io::load_qe` and `qe_run`'s
`--override NAME=VALUE` path log them at `warn`; the per-cell evaluators used
by sweeps and walk-forward deliberately drop them, because a warning
repeated ten thousand times is a warning nobody reads.

There is one lint today: `rolling_vol` applied directly to a bare
price variable (`close` / `open` / `high` / `low`). `rolling_vol`
returns the std-dev of whatever it is fed, so that call measures
dollars, not volatility. The message names `realized_vol` as the
replacement and `price_level(x)` as the way to say "I meant the level"
— see
[`qe-language.md`](qe-language.md#volatility-rolling_vol-vs-realized_vol).
`rolling_zscore` is deliberately excluded: it is dimensionless and
does not carry the defect.

Lint messages use the same `expression: column N: ...` wire format as
bind errors, so `load_qe`'s `rewrite_column_to_line_col` turns the
byte offset into a `line L col C` pair.
...
`qe::dsl::min_history(bound)` — one argument, a `const BoundAst&` — walks the AST and returns the maximum
...
- String-literal *escapes* (`\n`, `\"`, …). Plain string literals
  themselves are supported and required — `predict_return`'s
  `method = "ridge" | "lasso" | "gbdt"` and `model = "<path>"` kwargs
  take them, and the signal layer rejects strings everywhere else
