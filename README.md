...
Prebuilt macOS builds are at
**<https://qe.jiucheng-zang.ca/download/>** — Apple Silicon only, and
not notarized, so macOS blocks the first launch. That page has the
one-line fix and the checksums.

The binaries themselves are hosted on Cloudflare R2 at
`dl.jiucheng-zang.ca`, not as GitHub Release assets: a tag-based
release would auto-attach source archives of a repository that is
